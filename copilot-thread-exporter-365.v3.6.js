/**
 * v3.6.9 — FastScan Ordered + Deep Sanitize + JSONL (EN)
 * - Keeps your v3.6 behavior, but ALL strings are English (no ES/IT).
 * - Clean thread title: first “human” line from the first 'user' message
 *   (filters commands/paths/noise), truncated to 140 chars with “…”.
 * - Two dates (both via STRICT prompts; user must confirm):
 *   • Thread created (inside): ALWAYS ask (prefill = export date).
 *   • Last modified (Copilot list): default = bottom-most date chip found in the page,
 *     then ask for confirmation (strict YYYY-MM-DD).
 * - Strict validation: digits + hyphens only, 10 chars, real calendar date;
 *   if a chip lacks a year and resolves to a future date → shift to previous year.
 * - Exports HTML + JSONL sidecar with: thread_title, thread_created_date, thread_last_modified_date, etc.
 */
...
(async function () {
  ...
  const CFG = {
    ...
    EXCLUDE_PATTERNS: [ /* /text to exclude/i */ ],
    STOP_AT_TEXT: null,

    // Visible metadata
    SHOW_METADATA: true, INCLUDE_URL_TITLE: true,

    // Manual hints (to bypass prompts if desired)
    THREAD_DATE_HINT: null,        // created (YYYY-MM-DD)
    THREAD_LAST_MOD_HINT: null,    // last modified (YYYY-MM-DD)
  ...
  // Light UI noise remover for code/text
  ...
  // =======================
  // 3) Date parsing (it/es/en + Today/Yesterday)
  // =======================
  const MONTHS = {
    it:{full:["gennaio","febbraio","marzo","aprile","maggio","giugno","luglio","agosto","settembre","ottobre","novembre","dicembre"],
        short:["gen","feb","mar","apr","mag","giu","lug","ago","set","ott","nov","dic"], today:["oggi"], yesterday:["ieri"]},
    es:{full:["enero","febrero","marzo","abril","mayo","junio","julio","agosto","septiembre","setiembre","octubre","noviembre","diciembre"],
        short:["ene","feb","mar","abr","may","jun","jul","ago","sep","set","oct","nov","dic"], today:["hoy"], yesterday:["ayer"]},
    en:{full:["january","february","march","april","may","june","july","august","september","october","november","december"],
        short:["jan","feb","mar","apr","may","jun","jul","aug","sep","sept","oct","nov","dec"], today:["today"], yesterday:["yesterday"]}
  };
  const MONTH_WORDS = Array.from(new Set([]
    .concat(MONTHS.it.full,MONTHS.it.short,MONTHS.es.full,MONTHS.es.short,MONTHS.en.full,MONTHS.en.short)));
  const TODAY_RX = new RegExp(`\\b(?:${[].concat(MONTHS.it.today,MONTHS.es.today,MONTHS.en.today).join('|')})\\b`,'i');
  const YEST_RX  = new RegExp(`\\b(?:${[].concat(MONTHS.it.yesterday,MONTHS.es.yesterday,MONTHS.en.yesterday).join('|')})\\b`,'i');
  const RX_MONTH_DAY = new RegExp(`\\b(${MONTH_WORDS.join('|')})\\b\\s*(?:[,./-])?\\s*(\\d{1,2})\\b`,'i');
  const RX_DAY_MONTH = new RegExp(`\\b(\\d{1,2})\\s*(?:[,./-])?\\s*\\b(${MONTH_WORDS.join('|')})\\b`,'i');

  function monthIndexByWord(w){
    const wl=(w||'').toLowerCase();
    const sets=[MONTHS.it.full,MONTHS.it.short,MONTHS.es.full,MONTHS.es.short,MONTHS.en.full,MONTHS.en.short];
    for (const arr of sets){
      const idx=arr.findIndex(m=>wl===m);
      if (idx>= 0){
        if (arr===MONTHS.es.short && wl==='set') return 8;   // normalize "set" → Sep
        if (arr===MONTHS.en.short && wl==='sept') return 8;  // normalize "sept" → Sep
        return idx;
      }
    }
    return null;
  ...
  function yearRule(d){
    // If resolved date would be in the future vs. today (UTC), shift to previous year.
    const now=new Date();
    const todayUTC=new Date(Date.UTC(now.getUTCFullYear(),now.getUTCMonth(),now.getUTCDate()));
    if (d.getTime() > todayUTC.getTime()) d.setUTCFullYear(todayUTC.getUTCFullYear()-1);
    return d;
  }

  function inferISOFromLabel(label){
    const txt=(label||'').trim().toLowerCase();
    const now=new Date();
    const baseUTC=new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));

    if (TODAY_RX.test(txt)) return baseUTC.toISOString();
    if (YEST_RX.test(txt)) { const y=new Date(baseUTC); y.setUTCDate(y.getUTCDate()-1); return y.toISOString(); }

    let m=txt.match(RX_MONTH_DAY);
    if (m){
      const mi=monthIndexByWord(m[1]); const d=parseInt(m[2],10);
      if (mi!=null && d>=1 && d<=31) return yearRule(new Date(Date.UTC(baseUTC.getUTCFullYear(),mi,d))).toISOString();
    }
    m=txt.match(RX_DAY_MONTH);
    if (m){
      const d=parseInt(m[1],10); const mi=monthIndexByWord(m[2]);
      if (mi!=null && d>=1 && d<=31) return yearRule(new Date(Date.UTC(baseUTC.getUTCFullYear(),mi,d))).toISOString();
    }
    return null;
  }

  // Deep traversal (includes shadow DOM)
  function allElementsDeep(root){
    ...
    while(st.length){
      ...
      const sr=n.shadowRoot; if (sr) st.push(sr);
    }
    return out;
  }
  function textsFromEl(el){
    const out=[];
    try{
      const t=(el.innerText||el.textContent||'').trim(); if (t) out.push(t);
      const a=el.getAttribute?.('aria-label'); if (a) out.push(a);
      const ti=el.getAttribute?.('title'); if (ti) out.push(ti);
      for (const attr of Array.from(el.attributes||[]))
        if ((attr.name||'').startsWith('data-')) out.push(attr.value||'');
      try{ const b=getComputedStyle(el,'::before')?.content; if (b && b!=='none' && b!=='normal') out.push(b.replace(/^"(.*)"$/,'$1')); }catch{}
      try{ const a2=getComputedStyle(el,'::after')?.content; if (a2 && a2!=='none' && a2!=='normal') out.push(a2.replace(/^"(.*)"$/,'$1')); }catch{}
    }catch{}
    return out.filter(Boolean);
  }
  // Top-most (near header) and bottom-most (near end) date chips
  function pickTopDateChip(rootForY){
    const nodes=allElementsDeep(document); let best=null;
    for (const el of nodes){
      const strings=textsFromEl(el); let matched='';
      for (const s of strings){ const t=s.trim();
        if (TODAY_RX.test(t)||YEST_RX.test(t)||RX_MONTH_DAY.test(t)||RX_DAY_MONTH.test(t)){ matched=t; break; } }
      if (!matched) continue;
      let yTop=Infinity;
      try{
        const baseRect=rootForY.getBoundingClientRect?.()||document.documentElement.getBoundingClientRect?.()||{top:0};
        const r=el.getBoundingClientRect();
        const baseScrollTop=(rootForY===document.body||rootForY===document.documentElement)?(document.scrollingElement?.scrollTop||0):(rootForY.scrollTop||0);
        yTop=baseScrollTop+(r.top-baseRect.top);
      }catch{}
      if (!best || yTop < best.yTop) best={text:matched, yTop};
    }
    return best;
  }
  function pickBottomDateChip(rootForY){
    const nodes=allElementsDeep(document); let best=null;
    for (const el of nodes){
      const strings=textsFromEl(el); let matched='';
      for (const s of strings){ const t=s.trim();
  ...
  // =======================
  // 4) Selectors, sanitization, collection
  // =======================
  const SEL_ASSISTANT='[data-testid="markdown-reply"]';
  const SEL_USER='.fai-UserMessage__message, [data-testid="user-message"]';
  const MSG_SELECTORS=[SEL_ASSISTANT, '.fai-UserMessage__message', '[data-testid="user-message"]'];
  ...
  function detectRole(n){
    try{
      ...
      if (n.matches?.('.fai-UserMessage__message') || n.matches?.('[data-testid="user-message"]')) return 'user';
    }catch{}
    return 'unknown';
  }
